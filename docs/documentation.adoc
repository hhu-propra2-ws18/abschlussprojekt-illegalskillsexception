...
rented the item.
When a solution was found, the support staff will enter the amount of money
that should be paid by the affected parties, and the webapp will handle the
rest of the billing.
...
exception. Our solution was to use "Recover" methods in which we return null.
In the "Retryable" methods we then check for that null
and then throw our "ProPayConnectionException". For one case that was not
possible because the return type is void. We chose that solution
because it allows us to resolve the timeouts and retries almost only in the
ProPayService instead of the Controllers and other
Services.
